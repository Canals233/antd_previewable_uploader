...
export const vividImageTypes = ["image/jpeg", "image/png"];
export const vividVideoTypes = [
	"video/mp4",
	"video/avi",
	"video/mov",
	"video/wmv",
...
export const testVividFile = (file: RcFile) => {
	//测试文件是否符合要求
...
export const getDisplayableFileList = (rawList: any[]) => {
...
export const setListOnUploadChange = (
	{ fileList, file }: any,
...
export const playableImageRender = (originNode: any) => {
...
//这个是在ProFormUploadDragger里面的itemRender
export const playableUploadItemRender = (originNode: any, file: any) => {
	...
	if (file?.type?.includes("video") || file?.name?.endsWith("flv")) {
		...
		if (file.type === "video/mp4") {
			...
			originNode.props.children[0] = (
				<video key={Math.random()} width={85} height={100} src={url} />
			);
		} else
			originNode.props.children[0] = (
		...
		originNode.props.children[0] = (
			<video key={Math.random()} width={85} height={80} src={remoteUrl} />
		);
	}
	return originNode;
};

export const pastedFileFormat = (file: File) => {
	const rcFile = {
